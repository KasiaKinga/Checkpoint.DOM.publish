...
function updateCoffeeView(coffeeQty) {
  const coffeCounter = document.querySelector('#coffee_counter');
  coffeCounter.innerText = coffeeQty;
  return coffeCounter.innerText
}
...
function deleteAllChildNodes(parent) {
  let arrWithChilNodes = Array.from(parent.childNodes);
  return arrWithChilNodes.forEach(el => {
    parent.removeChild(el)
  })
}

function renderProducers(data) {
  // 3 func written previously
  const producerContainer = document.getElementById('producer_container');
  producerContainer.innerHTML = '';
  unlockProducers(data.producers, data.coffee);
  const unlockedProducers = getUnlockedProducers(data)

  unlockedProducers.forEach(el => {
    producerContainer.appendChild(makeProducerDiv(el));
  })
}
...
function getProducerById(data, producerId) {
  for(let element of data.producers) {
...
function updateCPSView(cps) {
  const cpsContainer = document.getElementById('cps');
